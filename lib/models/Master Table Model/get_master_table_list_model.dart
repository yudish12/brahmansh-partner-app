...
import 'assistant/assistant_all_skill_model.dart';
import 'assistant/assistant_language_list_model.dart';
import 'assistant/assistant_primary_skill_model.dart';
...
class GetMasterTableDataModel {
  ...
  factory GetMasterTableDataModel.fromJson(Map<String, dynamic> json) {
    try {
      return GetMasterTableDataModel(
        ...
        assistantPrimarySkill: List<AssistantPrimarySkillModel>.from(
            json["skill"].map((x) => AssistantPrimarySkillModel.fromJson(x))),
        assistantAllSkill: List<AssistantAllSkillModel>.from(
            json["skill"].map((x) => AssistantAllSkillModel.fromJson(x))),
        language: List<LanguageModel>.from(
            json["language"].map((x) => LanguageModel.fromJson(x))),
        assistantLanguage: List<AssistantLanguageModel>.from(
            json["language"].map((x) => AssistantLanguageModel.fromJson(x))),
  ...
  Map<String, dynamic> toJson() => {
        ...
        "assistantPrimarySkill":
            List<dynamic>.from(assistantPrimarySkill!.map((x) => x.toJson())),
        "assistantAllSkill":
            List<dynamic>.from(assistantAllSkill!.map((x) => x.toJson())),
        "assistantLanguage":
            List<dynamic>.from(assistantLanguage!.map((x) => x.toJson())),
        "mainSourceBusiness":
