...
import '../../models/astrologerAssistant_model.dart';
import '../../services/apiHelper.dart';
import '../../views/HomeScreen/Assistant/assistant_screen.dart';
...
//Date oF Birth
  ...
  FocusNode fBirthDate = FocusNode();
...
//Validate Add my assistant
  validateAssistantForm() {
    try {
      if (userFile != null &&
        ...
        global.showToast(
            message: "Something Wrong in astrologer assistant Form");
      }
    } catch (e) {
            ...
            await apiHelper.astrologerAssistantAdd(assistant).then(
              (apiResult) async {
                if (apiResult.status == '200') {
                  ...
                  await getAstrologerAssistantList();
                  global.showToast(
                      message: "You have succesfully add astrologer assistant");
    ...
    try {
      ...
      if (assistant.birthdate != null) {
        ...
        cBirthDate.text = DateFormat('dd-MM-yyyy')
            .format(DateTime.parse(assistant.birthdate.toString()));
      }
      ...
      //Primary skill
      if (assistant.assistantPrimarySkillId != null &&
          assistant.assistantPrimarySkillId != []) {
        cAssistantPrimarySkill.text = assistant.assistantPrimarySkillId!
            .map((e) => e.name)
            .toList()
            .toString()
            .replaceAll('[', '')
            .replaceAll(']', '');
        for (var i = 0;
            i < global.assistantPrimarySkillModelList!.length;
            i++) {
          global.assistantPrimarySkillModelList![i].isCheck = false;
          for (var j = 0; j < assistant.assistantPrimarySkillId!.length; j++) {
            if (assistant.assistantPrimarySkillId![j].id ==
                global.assistantPrimarySkillModelList![i].id) {
      ...
      //All skill
      if (assistant.assistantAllSkillId != null &&
          assistant.assistantAllSkillId != []) {
        cAssistantAllSkill.text = assistant.assistantAllSkillId!
            .map((e) => e.name)
            .toList()
            .toString()
            .replaceAll('[', '')
        ...
        for (var i = 0; i < global.assistantAllSkillModelList!.length; i++) {
          ...
          for (var j = 0; j < assistant.assistantAllSkillId!.length; j++) {
            if (assistant.assistantAllSkillId![j].id ==
                global.assistantAllSkillModelList![i].id) {
      ...
      //Language
      if (assistant.assistantLanguageId != null &&
          assistant.assistantLanguageId != []) {
        cAssistantLanguage.text = assistant.assistantLanguageId!
            .map((e) => e.name)
            .toList()
            .toString()
            .replaceAll('[', '')
        ...
        for (var i = 0; i < global.assistantLanguageModelList!.length; i++) {
          ...
          for (var j = 0; j < assistant.assistantLanguageId!.length; j++) {
            if (assistant.assistantLanguageId![j].id ==
                global.assistantLanguageModelList![i].id) {
  ...
  updateAstrologerAssistant(int id) async {
    try {
            ...
            await apiHelper.astrologerAssistantUpdate(assistant).then(
              (apiResult) async {
                if (apiResult.status == '200') {
                  ...
                  global.showToast(
                      message:
                          "You have succesfully update astrologer assistant");
                } else if (apiResult.status == '400') {
      ...
      print(
          "Exception - $screen - updateAstrologerAssistant(): " + e.toString());
    }
  }
  ...
  Future getAstrologerAssistantList() async {
    try {
      ...
      print(
          'Exception: $screen - getAstrologerAssistantList(): ' + e.toString());
    }
  }
